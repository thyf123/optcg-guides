...
let LEADERS = {
  ...
  op01_law: {
    ...
    { name:"OP12 Mirror", warn:false, go:"?", wr1:null, wr2:null, style:"—", deck:"rosinante", essential:[], tips:[] },
...
function getLM() {
  ...
  // Pre-seed seenCardId with cardIds of covered entries so alt-key dupes (e.g. op12rosinante) are suppressed
  const seenCardId = new Set(
    [...covered].map(dk => LEADERS[dk]?.cardId).filter(Boolean)
  );
